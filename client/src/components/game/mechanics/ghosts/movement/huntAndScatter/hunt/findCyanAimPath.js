export default function findCyanAimPath(pacman, variables, redGhost, pathway) {
  ...
  if (pacman.rotation === 0) x += variables.length * 2;
  else if (pacman.rotation === Math.PI / 2) y += variables.length * 2;
  else if (pacman.rotation === Math.PI) x -= variables.length * 2;
  else if (pacman.rotation === (Math.PI * 3) / 2) y -= variables.length * 2;
  return {
